...
대게의 버전 관리 시스템처럼, Git도 어떤 중요한 액션이 일어날 때 사용자 스크립트를 수행시킬 방법이 있습니다. 이런 훅(Hook)은 두 유형이 있는데 클라이언트 측과 서버 측입니다. 클라이언트 측 훅은 커밋과 머지와 같은 클라이언트 연산을 위한 것입니다. 서버 측 훅은 푸쉬된 커밋을 받는 것과 같은 서버 연산을 위한 것이지요. 이런 훅은 여러 용도로 사용될 수 있습니다. 이 장에선 이 중 몇 가지를 익히겠습니다.
